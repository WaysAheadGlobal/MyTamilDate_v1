...
import { Container, ModalBody, Spinner } from "react-bootstrap";
import { FaAngleDown } from "react-icons/fa6";
const Forms = {
  Radio: ({ options, value, setValue, firstOption, selected }) => {
    const [currentValue, setCurrentValue] = useState(
      value || selected || firstOption?.value || null
    );
        ...
        {firstOption && (
          <label htmlFor={firstOption.label} className={styles.inputRadio}>
            <input
              ...
              checked={
                currentValue === firstOption.value || currentValue === "any"
              }
            ...
            >
              <input
                ...
                checked={
                  option.id === null
                    ? currentValue === null
                    : currentValue === option.id || currentValue === option.name
                }
  ...
  Range: ({ value, setValue, age_from, age_to }) => {
    ...
    return (
      <>
        ...
        >
          {value?.[0] ?? age_from} - {value?.[1] ?? age_to}
        </p>
        <Slider
  ...
  Location: ({ options, value, setValue, locationId }) => {
    ...
    useEffect(() => {
      if (
        ...
        options[selectedCountry].length > 0 &&
        !selectedCity
      ) {
      ...
      <>
        <label htmlFor="country" className={styles.locationRadio}>
          ...
          <input
            ...
            id="country"
            name="country-option"
            checked={!countrySelectCollasped}
            onClick={() => setCountrySelectCollasped(false)}
          />
        </label>
        <label htmlFor="any" className={styles.locationRadio}>
          ...
          <input
            ...
            value="any"
            checked={countrySelectCollasped}
            onClick={() => setCountrySelectCollasped(true)}
          />
        </label>
        {!countrySelectCollasped && (
          <>
  ...
  Select: ({ options, value, setValue, firstOption, selected }) => {
    ...
    useEffect(() => {
      if (!selected) {
        ...
        setSelectedOption(
          options.find((option) => option.name === selected)?.id
        );
      }
    }, [selected]);
    ...
    return (
      <div className={styles["select-container"]} data-open={open}>
        <div
          className={styles["select-display"]}
          onClick={() => {
            setOpen(!open);
          }}
          data-value={
        ...
        <div
          className={styles["select-backdrop"]}
...
export default function Preferences() {
  ...
  useEffect(() => {
    if (selectedPreference.element !== "location") {
      ...
      if (selectedPreference.element === "gender") {
        ...
        if (
          selectedPreference.element === "children" ||
          selectedPreference.element === "family"
        ) {
          console.log("donee");
          ...
          const filteredOptions = options.filter(
            (option) => option.name !== "Prefer not to say"
          );
          ...
          if (selectedPreference.element === "religion") {
            ...
            const options = selectedPreferenceOptions.filter(
              (option) => option.name !== "Prefer not to say"
            );
  ...
  useEffect(() => {
    ...
    (async () => {
      const response = await fetch(
        `${API_URL}customer/user/preferences/options/${selectedPreference.optionsApiEndpoint}`,
        {
          headers: {
            Authorization: `Bearer ${cookies.getCookie("token")}`,
          },
  ...
  async function handlePreferenceSave(e) {
    ...
    try {
      const response = await fetch(
        `${API_URL}customer/user/preferences/save/${selectedPreference.saveApiEndpoint}`,
        {
          method: "PUT",
          headers: {
                  ...
                  firstOption={
                    selectedPreference.optionsApiEndpoint !== "genders"
                      ? {
                          value: "any",
                          label:
                            selectedPreference.optionsApiEndpoint === "location"
                              ? "Any"
                              : "Open to all",
                        }
                      : null
      ...
      >
        ...
        <div className={styles.preferences}>
          ...
          <div className={styles.option}>
            ...
            >
              ...
              <g clip-path="url(#clip0_778_16194)">
                <path
                  d="M24.2984 10.7012H11.6984C10.7043 10.7012 9.89844 11.5071 9.89844 12.5012V25.1012C9.89844 26.0953 10.7043 26.9012 11.6984 26.9012H24.2984C25.2925 26.9012 26.0984 26.0953 26.0984 25.1012V12.5012C26.0984 11.5071 25.2925 10.7012 24.2984 10.7012Z"
                  stroke="#6C6C6C"
                  stroke-width="1.4"
          ...
          <div className={styles.option}>
            ...
            >
              {preferences?.religion ?? "Open to all"}
            </p>
          </div>
          ...
          >
            ...
            <div className={styles.option}>
              ...
              >
                ...
                />
                <path
                  d="M18 27C22.4183 25 26 21.4183 26 17C26 12.5817 22.4183 9 18 9C13.5817 9 10 12.5817 10 17C10 21.4183 13.5817 25 18 27Z"
                  stroke="#6C6C6C"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
              ...
              <p
                ...
                // })}
              >
                {preferences?.location ?? "Open to all"}
              </p>
            </div>
            <div className={styles.option}>
              <svg
              ...
              >
                {preferences?.education ?? "Open to all"}
              </p>
            </div>
            <div className={styles.option}>
              <svg
              ...
              >
                {preferences?.have_kids ?? "Open to all"}
              </p>
            </div>
            <div className={styles.option}>
              <svg
              ...
              >
                {preferences?.want_kids ?? "Open to all"}
              </p>
            </div>
            ...
            <div className={styles.option}>
              ...
              >
                {preferences?.smoking ?? "Open to all"}
              </p>
            </div>
            <div className={styles.option}>
              <svg
              ...
              >
                {preferences?.drinking ?? "Open to all"}
              </p>
            </div>
          </div>
          <Button
...
const LocationModal = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await fetch(
          `${API_URL}customer/user/preferences/options/locations`,
          {
            headers: {
              Authorization: `Bearer ${cookies.getCookie("token")}`,
            },
  ...
  const getLocationData = () => {
    ...
    if (selectedCountry) {
      return {
        location_id: selectedPreferenceOptions[selectedCountry][0]?.id,
        type: 1,
      }; // Country selected, use first city id
    }
  ...
  async function handlePreferenceSave(e) {
    ...
    try {
      const response = await fetch(
        `${API_URL}customer/user/preferences/save/location`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleMatchingCountry = () => {
    if (countrySearch) {
      selectedPreferenceOptions.forEach((country) => {
        if (country.toLowerCase() === countrySearch.toLowerCase()) {
          setSelectedCountry(country);
          // setErrorMessage("");
        }
      });
  ...
  const handleMatchingCity = () => {
    console.log("City Search:", citySearch);
    if (citySearch) {
      options[selectedCountry].forEach((city) => {
        // console.log("City:", city);
        if (city.location_string) {
          if (city.location_string.toLowerCase() === citySearch.toLowerCase()) {
        ...
        {countryOptions && (
          <>
            ...
            <Container
              ...
              className="located-country collapsed"
            >
              <div
                style={{
                  display: "flex",
                ...
                onClick={() => {
                  countrySelectRef.current.classList.toggle("collapsed");
                }}
              >
                <input
                  type="text"
                  placeholder="Select a country"
                  value={toLowerCase(countrySearch)}
                  onChange={(e) => {
                    setCountrySearch(toLowerCase(e.currentTarget.value));
                    ...
                    <>
                      <div
                        className={`scroll-item ${
                          selectedCountry === "Canada" ? "selected" : ""
                        }`}
                        onClick={() => {
                          handleCountrySelect("Canada");
                          setCountrySearch("Canada");
                          countrySelectRef.current.classList.add("collapsed");
                        }}
                      >
                        {"Canada"}
                      </div>
                      ...
                      <div
                        className={`scroll-item ${
                          selectedCountry === "United Kingdom" ? "selected" : ""
                        }`}
                        onClick={() => {
                          ...
                          .map((country) => (
                            <div
                              ...
                              onClick={() => {
                                ...
                                countrySelectRef.current.classList.add(
                                  "collapsed"
                                );
                              }}
                            >
      ...
      .map((option) => (
        <MenuItem key={option.id} value={option.id}>
          {option.location_string}
            ...
            {selectedCountry && (
              <Container ref={citySelectRef} className="located-city collapsed my-2">
                <div
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
                ...
                <div className="scroll-container-vertical">
                  {!citySearch &&
                    ...
                    options[selectedCountry]
                      ...
                      .sort((a, b) =>
                        a.location_string?.localeCompare(b.location_string)
                      )
                      .map((city) => (
                        <div
                          key={city.id}
