...
export default function PersonalityProfile() {
    ...
    useEffect(() => {
        ...
        fetch(`${API_URL}/customer/users/personality-options`, {
            ...
            .catch(error => console.error('Error fetching personality options:', error));
    }, []);

    useEffect(() => {
        // Fetch selected personalities
        fetch(`${API_URL}/customer/users/personalities`, {
            ...
            .catch(error => console.error('Error fetching selected personalities:', error));
    }, [personalities]);
                    ...
                    <Container className='job-content'>
                        <Container className='job-details'>
                            <div className='your-job'>
                                ...
                                <InputGroup className='job-search-bar' style={{ overflow: "hidden" }}>
                                    <Form.Control
                                    ...
                                    />
                                    <InputGroup.Text className='search-icon' style={{ position: "relative", top: "0rem" }}>
                                        <IoIosSearch />
                                    </InputGroup.Text>
                                </InputGroup>
                                <div>
                                    {selectedPersonalities.length < 3 || selectedPersonalities.length > 8 ? (
                                        <p style={{ fontSize: "16px", fontWeight: "600", color: "#4E1173", lineHeight: "18.5%", marginBottom: "20px" }}>
                                            Select 3-8 options
                                        </p>
                                    ) : null}
                                </div>
                                    ...
                                    style={{
                                        ...
                                        {filteredPersonality.map((personality, index) => (
                                            <div
                                                ...
                                                onClick={() => {
                                                    if (selectedPersonalities.find(p => p.name === personality.name)) {
                                                        setSelectedPersonalities(selectedPersonalities.filter(p => p.name !== personality.name));
                                                    } else {
                                                        setSelectedPersonalities([...selectedPersonalities, personality]);
                                                    }
                                                }}
